...
if len(t_spec)>0:
    ...
    np.save('./SDSS_PAC/%s_%.1f_%.1f_results_jack.npy' % (zs[k],ms[spec]+0.1,ml[photo]),cl)
    np.save('./SDSS_PAC/%s_%.1f_%.1f_param_jack.npy' % (zs[k],ms[spec]+0.1,ml[photo]),np.array([len(t_spec)]))
